...
def fetch_news_pure_scraping(topic: str, location: str, time_range_code: str, max_results: int):
    ...
    try:
        ...
        for entry in news_items:
            ...
            # 2. Sub judul & Deskripsi diambil dari potongan summary RSS
            cleaned_summary = clean_html(summary_raw)
            
            # Sub judul (Ringkas maks 25 kata)
            ...
            # Deskripsi Lengkap (Lebih panjang maks 80 kata)
            deskripsi = truncate_words(cleaned_summary, max_words=80)
            if not deskripsi or deskripsi == "...":
                deskripsi = f"Simak berita selengkapnya mengenai topik {topic} langsung melalui tautan sumber resmi."
...
def export_to_excel(entries, filepath):
    ...
    try:
        ...
        # Header baru (ditambahkan kolom Deskripsi)
        headers = ["Judul", "Sub judul", "Deskripsi", "Sumber Nama Website", "Foto", "Sumber"]
        ws.append(headers)
        ...
        for row_num, entry in enumerate(entries, 2):
            ...
            for col_num in range(1, len(headers) + 1):
                ...
                # Alignment & Wrap Text rules untuk kolom Deskripsi baru
                if col_num in [1, 2, 3]: # Judul, Sub judul, & Deskripsi
        ...
        # Set tinggi baris header & data (Tinggi data dinaikkan agar Deskripsi muat rapi)
        ws.row_dimensions[1].height = 28
        for row in range(2, len(entries) + 2):
            ws.row_dimensions[row].height = 55
            
        # Atur lebar kolom optimal (Kolom 3 / Deskripsi diberikan ukuran lebar 50)
        column_widths = {1: 35, 2: 35, 3: 50, 4: 22, 5: 20, 6: 30}
...
def main():
    print("=" * 65)
    print("        NEWS SCRAPER TO EXCEL & JSON (WITH DESCRIPTION)    ")
    print("=" * 65)
